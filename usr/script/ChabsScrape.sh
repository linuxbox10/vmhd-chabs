...
# to query and download random images for Sky show bouquet screen.
#
...
# (Allows invocation via SkyPicks.sh for out-of-date file updating).
#
...
#          could be used in SkyPicks.sh in order to prevent excessive d/ls.
...
workdir="/usr/script/scrapes"		# PREFER: /usr/share/enigma2/VM-HD/TopPicks/scrapes
